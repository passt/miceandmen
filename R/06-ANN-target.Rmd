...
Before running this Markdown go back and run __04-pre-Processing-for-ML.Rmd__ and __05-ANN-source.Rmd__.
...
for (fold in 1:ksplits){
  for (i in 1:15)
  lledom[[i]][[fold]] = keras::load_model_hdf5(filepath = paste0('2019-10-24-ML_NN_1hidden1softmax_Model', fold, '.hdf5'))
}
Sys.time()
```
...
Exclude cells that have been identified as endothelial cells, megakaryocytes or basophils, in order to avoid changing weights reflecting the impurity of training data.
...
Sample 1, 2, 4, 8, 16, 32 for re-training. Keep track of which ones used.
...
### Save models (target, transfer, ANN)

```{r}
# # Save Models (ANN transfer)
# Sys.time()
# for (fold in 1:ksplits){
#   for (i in 1:15)
#   keras::save_model_hdf5(lledom[[i]][[fold]],
#             filepath = paste0('../Models/Target/ANN/transfer/Target-ANN-transfer-Model', fold, '_n',nExmpl[i], '.hdf5'),
#             overwrite = FALSE)
# }
# Sys.time()
# 
# # indices
# save(htestix, h.kfold, htrainix, lledom.predict, lledom.predict.p, file='../Models/Target/ANN/transfer/Target-ANN-transfer-xValSplits.RData')
```


### Load models (target, transfer, ANN)
...
for (fold in 1:ksplits){
  for (i in 1:15)
  lledom_test[[i]][[fold]] = keras::load_model_hdf5(filepath =
    paste0('../Models/Target/ANN/transfer/Target-ANN-transfer-Model', fold, '_n',nExmpl[i], '.hdf5'))
}
Sys.time()

load('../Models/Target/Target-xValSplits.RData')
...
```{r}
heatmap(metricsTL['balanced Accuracy', , , ] %>% apply(c(1,3), mean),
...
col <- sapply(unique(hBMMNC@meta.data$NN_predict.lledom),
       function(x) { as.vector(mAnnot.res1.1$DPcolor[mAnnot.res1.1$CellType == x][1])})
col['training'] <- NA
col['validation'] <- NA
# col['unrepresented'] <- NA


# match colors from mBMMNC annotation.1.1
aplot <- TSNEPlot(object = hBMMNC, do.return = T, do.label = F,  no.legend=T,
                  group.by = 'NN_predict.consensus.lledom',
                  colors.use = col, pt.size = 0.5,
                  cells.use = names(hBMMNC@ident)[sample(ix.test.hs, length(ix.test.hs))]) #1:9394, 9394)])#
aplot + theme(aspect.ratio=1) + coord_fixed()
```


# Train models from random
...
for (fold in 1:ksplits) {
  ...
  for (i in 1:15) {
    ...
    # # Define the model
    # inputs <- layer_input(shape = ncol(x.human))
    # predictions <- inputs %>%
    ...
    # 
    # # Put pieces together
    # modellOA[[i]][[fold]] <- keras_model(inputs = inputs, outputs = predictions)
    # 
    # # Compile the model
...
#                          filepath = paste0('../Models/Target/ANN/random/Target_ANN_random_Model', fold, '_n', nExmpl[i], '.hdf5'),
#                          overwrite = TRUE)
# }
# Sys.time()
```

### Load models (target, random, ANN)
...
sapply(1:15, function(i) {t.test(x=atl[,i], y=aoa[,i], paired = T, alternative='greater')$p.value %>% p.adjust(method='BH') %>% -log10(.)}) %>%
  plot(x=nExmpl, y=., ylim=c(0,7), las=1, type='b', cex=.75, col='cyan')
axis(1, at=nExmpl,las=1)
...
heatmap(datTL, Rowv = ctyp, #metricsTL['F1', , , ] %>% apply(c(1,3), mean, na.rm=T),
        revC = T,  scale='none', Colv = NA, las=1, 
...
plot(x=metricsTL['F1',,,1] %>% apply(1, mean), y=metricsTL['F1',,,15] %>% apply(1, mean), xlim=c(0.25,1), ylim=c(0.6,1), col=col, pch=19, las=1, xlab='A', ylab='C'); abline(h = .925, v=.5, lty=3)
